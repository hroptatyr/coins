...
#define API_HOST	"api.poloniex.com"
#define API_PORT	443
...
static ssize_t
send_masked(ssl_ctx_t ss, uint8_t code, const uint8_t *data, size_t dlen)
{
	size_t loff = 0U;
	wsfr_t fr = {
		.code = code,
	...
	if (dlen >= 65536U) {
		fr.plen = 127U;
		fr.plen64 = htobe32((uint64_t)dlen >> 32U);
		fr.plen32 = htobe32(dlen & 0xffffffffU);
		slen = sizeof(wsfr_t);
	} else if (dlen >= 126U) {
		fr.plen = 126U;
		fr.plen16 = htobe16(dlen);
...
{
	send_masked(ss, 0xaU, (const uint8_t*)pmsg, plen);
	ping--;
	toout_logline("PONG!", 5U);
	return;
}
...
{
	toout_logline(chan, clen);
	return send_masked(ss, 0x1U, (const uint8_t*)chan, clen);
}
...
ehlo_coin(EV_P_ coin_ctx_t ctx)
{
	...
	request_sub(ctx->ss, ehlo, strlenof(ehlo));
