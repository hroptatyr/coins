...
typedef struct fast_ctx_s *fast_ctx_t;
...
{
/* we know that w is part of the fast_ctx_s structure */
	fast_ctx_t ctx = w->data;
...

static void
subscr_fast(EV_P_ fast_ctx_t ctx)
{
...
\"filter\":[\"NMC\",\"SPH\",\"SD\",\"BK\",\"MA\"]\
	...
	ws_send(ctx->ws, flt, strlenof(flt), 0);
	for (size_t i = 0U; i < ctx->nsubs; i++) {
		int len = snprintf(buf, sizeof(buf), "\
{\"currency\":\"%s\"}", ctx->subs[i]);
...
static void
init_fast(EV_P_ fast_ctx_t ctx)
{
...
static void
deinit_fast(EV_P_ fast_ctx_t ctx)
{
...
static void
reinit_fast(EV_P_ fast_ctx_t ctx)
{
	fprintf(stderr, "REINIT\n");
	deinit_fast(EV_A_ ctx);
	init_fast(EV_A_ ctx);
	return;
}
...
prepare(EV_P_ ev_prepare *w, int UNUSED(revents))
{
	...
	case FAST_ST_UNK:
		...
		init_fast(EV_A_ ctx);
		if (ctx->st != FAST_ST_CONN) {
			break;
		}
	case FAST_ST_CONND:
		/* waiting for that HTTP 101 */
		subscr_fast(EV_A_ ctx);
		break;
...
static void
deinit_ev(EV_P_ fast_ctx_t ctx)
{
