...
typedef struct ff_ctx_s *ff_ctx_t;
...
struct ff_ctx_s {
	/* the hostname we're on about */
	struct sockaddr_storage sa_ws[1];
...

/* was an offsetof macro, hence the slot arg */
#define ff_ctx(ptr, slot)	((ptr)->data)

static void
ws_cb(EV_P_ ev_io *w, int UNUSED(revents))
{
/* we know that w is part of the fxstreet_ctx structure */
	ff_ctx_t ctx = ff_ctx(w, watcher);
...
subscr_ff(EV_P_ ff_ctx_t ctx)
{
	...
	if ((fd = open(ctx->jf, O_RDONLY)) < 0) {
		serror("cannot open join file `%s'", ctx->jf);
		return;
	} else if (fstat(fd, &st) < 0) {
		serror("cannot stat join file `%s'", ctx->jf);
		goto clo_out;
	} else if ((fz = st.st_size) <= 0) {
		errno = 0, serror("join file `%s' has illegal size", ctx->jf);
		goto clo_out;
	} else if ((jb = mmap(NULL, fz,
			      PROT_READ, MAP_SHARED, fd, 0)) == MAP_FAILED) {
		serror("cannot mmap join file `%s'", ctx->jf);
		goto clo_out;
	}
...
{
	static const char cf[] = "ff.cnf";
	yuck_t argi[1U];
	struct ff_ctx_s ctx[1U];
